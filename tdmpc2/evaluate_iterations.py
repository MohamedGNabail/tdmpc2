...
def evaluate(cfg: dict):
	...
	# --- init wandb ---
	wandb.init(project="tdmpc2-eval", config=cfg)

	# Loop over different iteration values
	for iters in [6, 12, 24, 48, 96]:
		cfg.iterations = iters
		print(colored(f"\nRunning evaluation with iterations={iters}", "cyan", attrs=["bold"]))
		wandb.log({"iterations": iters})
		...
		for task_idx, task in enumerate(tasks):
			...
			# log to wandb
			wandb.log({
				f"{task}/reward": ep_rewards,
				f"{task}/success": ep_successes,
				"iterations": iters
			})
