...
class RewardModel:
    ...
    def construct_ensemble(self):
        for i in range(self.de):
            model = nn.Sequential(*self.gen_net(in_size=self.ds+self.da, 
                                           ...
                                           activation=self.activation)).float().to(self.cfg.cuda_device)
    ...
    def r_hat_member(self, x, member=-1):
        ...
        return self.ensemble[member](torch.from_numpy(x).float().to(self.cfg.cuda_device))
    ...
    def get_train_acc(self):
        ...
        for epoch in range(num_epochs):
            ...
            labels = torch.from_numpy(labels.flatten()).long().to(self.cfg.cuda_device)
            total += labels.size(0)
            for member in range(self.de):
                # get logits
                ...
                if full_start < len(obs):
                    ...
                    for idx in range(len(full_obs) // batch_size + 1):
                        ...
                        if start < len(full_obs):
                            ...
                            dist = torch.norm(
                                obs[full_start:full_end, None, :].to(self.cfg.cuda_device) - full_obs[None, start:end, :].to(self.cfg.cuda_device), dim=-1, p=2
                            )
                            dists.append(dist)
